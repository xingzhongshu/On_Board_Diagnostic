...
void DynamixelClass::begin(long baud, int8_t directionPin)
{
	if(directionPin >= 0) Direction_Pin = directionPin;
...
void DynamixelClass::writeraw(uint8_t ID, uint8_t len, uint8_t* buf)
{
	uint8_t i, chksum;
...
#define WRITERAW_PRE(n)		uint8_t buf[n], *p = buf;
#define WRITERAW_POST()		writeraw(ID, sizeof(buf), buf);
...
// CMD=2: readdata
// CMD=3: writedata
int DynamixelClass::write(uint8_t ID, uint8_t addr, uint8_t d0)
{
}


// CMD=4: regwrite
#define REGWRITE_PRE(n)		WRITERAW_PRE(n); *p++ = AX_REG_WRITE; *p++ = addr;
#define REGWRITE_POST()		WRITERAW_POST()
void DynamixelClass::regwrite(uint8_t ID, uint8_t addr, uint8_t d0)
{
	REGWRITE_PRE(3);
	*p++ = d0;
	REGWRITE_POST();
}
void DynamixelClass::regwrite(uint8_t ID, uint8_t addr, uint8_t d0, uint8_t d1)
{
	REGWRITE_PRE(4);
	*p++ = d0;
	*p++ = d1;
...
void DynamixelClass::regwrite(uint8_t ID, uint8_t addr, uint8_t d0, uint8_t d1, uint8_t d2)
{
	REGWRITE_PRE(5);
	*p++ = d0;
	*p++ = d1;
...
void DynamixelClass::regwrite(uint8_t ID, uint8_t addr, uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3)
{
	REGWRITE_PRE(6);
	*p++ = d0;
	*p++ = d1;
...
{
	regwrite(ID, AX_GOAL_POSITION_L, Position, Position >> 8, Speed, Speed >> 8);
    return (read_error());               // Return the read error
}
